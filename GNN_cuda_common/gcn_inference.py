...
from .agg_ops import linear_forward_
from .gcn_ops import spmm_gcn_forward_
from .graph_utils import build_gcn_normalized_csr, resolve_dataset_path
...
def _make_layer_state(
    ...
    *,
    ...
    weight = allocate_like_mode(weight_cpu, memory_mode=memory_mode, device=device)
    bias = allocate_like_mode(bias_cpu, memory_mode=memory_mode, device=device)
    ...
    if is_uvm_mode(memory_mode):
        ...
        for tensor in (weight, bias, agg_buffer, out_buffer):
...
def run_gcn_inference(
    *,
    ...
    deg_inv_sqrt = allocate_like_mode(csr.deg_inv_sqrt, memory_mode=graph_memory_mode, device=device)
    features = allocate_like_mode(features_cpu, memory_mode=graph_memory_mode, device=device)
    if is_uvm_mode(graph_memory_mode):
        for tensor in (row_ptr, col_ind, deg_inv_sqrt):
            apply_managed_policy(
                tensor,
                device=device,
                preferred_location=managed_cfg.preferred_location,
    ...
    def forward_once() -> torch.Tensor:
        ...
        for idx, layer in enumerate(layer_states):
            ...
            with _nvtx_range(nvtx_enabled, f"{tag}/aggregation"):
                spmm_gcn_forward_(row_ptr, col_ind, deg_inv_sqrt, x, layer.agg_buffer)
